...
const config = { method: 'post', url: 'https://api.openai.com/v1/chat/completions', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + global.openai_key }, data: JSON.stringify({ 'model': 'gpt-3.5-turbo', 'messages': [{ role: 'system', content: 'this bot a ete cree par shizzy (andy) il est un developper qui vit a miami' }, ...chgptdb ]})}
...
try {
const botIA222 = await openaiii.createCompletion({ model: "text-davinci-003", prompt: text, temperature: 0.3, max_tokens: 4097, stop: ["Andy:", "Human:"], top_p: 1, frequency_penalty: 0.2, presence_penalty: 0, })
...
let akuariapiresult1 = await translate(`${akuariapijson1.respon}`, { to: 'fr', autoCorrect: true })
...
let hasill22_result = await translate(`${hasill22.result}`, { to: 'fr', autoCorrect: true })  
m.reply(`${hasill22_result.text}`.trim())         
} catch {
try {  
const searchString2 = " English "
const replacementString2 = ' french '
let rres = await fetch(`https://api.ibeng.tech/api/others/chatgpt?q=Hola&apikey=eMlBNRzUXv`)
let jjson = await rres.json()
let hahaha = await translate(`${jjson.data}`, { to: 'fr', autoCorrect: true })
...
let akuariapiresult2 = await translate(`${akuariapijson2.respon}`, { to: 'fr', autoCorrect: true })
m.reply(akuariapiresult2.text.trim())    
} catch {    
try {   
let syms1 = `this bot a ete cree par shizzy (andy) il est un developper qui vit a miami`  
...
let fgjson1_result = await translate(`${fgjson1.result}`, { to: 'fr', autoCorrect: true })  
m.reply(fgjson1_result.text.trim())    
} catch {    
}}}}}}}}
handler.command = ['ai', 'gpt', 'ia', 'robot']
